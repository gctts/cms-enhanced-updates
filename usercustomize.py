...
def _normalize_auto_update_manifest(manifest):
    ...
    if isinstance(targets, dict):
        ...
        if (
            ...
            and target_name
            and str(target_name).strip().lower().endswith(".so")
        ):
            generic_so_name = f"{str(target_name).split('.', 1)[0]}.so"
            target_payload = targets.get(generic_so_name)
...
def _get_auto_update_proxies():
    proxy = str(_get_cms_proxy() or "").strip()
...
def _fetch_auto_update_manifest():
    ...
    resp = requests.get(
        ...
        proxies=_get_auto_update_proxies(),
    )
...
def _download_auto_update_file(download_url, target_path, expected_sha256="", expected_size=0):
    ...
    proxies = _get_auto_update_proxies()
    try:
        with requests.get(download_url, timeout=timeout, stream=True, proxies=proxies) as resp:
            resp.raise_for_status()
